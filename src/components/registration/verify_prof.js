...
class verify_prof extends Component{
    constructor(props){
        super(props)
        this.state={
            verify:[]
    ...
    getDetails(){
        axios.get('http://localhost:4000/app/verify_prof')
        .then(res=>{
    ...
    renderTableData(){
        ...
        return this.state.verify.map((professor,index)=>{
            const {p_id,p_name,p_email,p_contact_no}=professor
            return(
                <tr id={p_id}>
                    <td>{p_id}</td>
                    <td>{p_name}</td>
                    ...
                    <td><button onClick={()=>{
                        Swal.fire({
                            title:'Verification',
                            text:'Are you sure, you want to Aprove professor '+p_id,
                            confirmButtonText:'Yes, Approve',
                            showCancelButton:true,
                            cancelButtonText:'No, dont'
                        }).then((result)=>{
                            if(result.value){
                                axios.post('http://localhost:4000/app/verify_professor',{p_id})
                        .then(Response=>{
                            if(Response.status===200){
                                document.getElementById("after_modify").innerHTML="Approved "+p_id;
                                
                                window.location.replace('/verify_prof')
                            }
                        })
                            }
